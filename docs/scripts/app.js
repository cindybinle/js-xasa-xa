...
function initToolbar() {
	...
	downloadLink.onclick = function () {
		document.title = downloadLink.download + ' - (paint.thuanitdn.com)';
		saveDialog.close();
	};
	...
	document.getElementById('upload').addEventListener('change', function (e) {
		if (window.File && window.FileReader && window.FileList && window.Blob) {
			...
			if (!file.type.match('image.*')) {
				alert('Paint Online can only open valid image files.');
				return;
			}
			...
			reader.onload = function (ev) {
				...
				document.title = fileName + '- (paint.thuanitdn.com)';
...
function initSettings() {
	...
	document.getElementById('colors').style.backgroundColor = "#ffffff";
...
window.addEventListener('load', function () {
	...
	// document.title = 'untitled.png - (paint.thuanitdn.com)'
